...
#include "Database/Mappers/session_mapper.h"
#include "Database/Postgres/Repositories/pqxx_session_repository.h"
#include <stdexcept>

PqxxSessionRepository::PqxxSessionRepository(IConnectionPool* connectionPoolObs)
...
std::vector<Session> PqxxSessionRepository::FindByUserId(const UserId& userId) {
  return TryFetchSessions(
    "SELECT * FROM sessions WHERE user_id = $1",
    QueryParams{}.BindParam(userId.ToString()),
    Transaction{ std::move(connection_pool_obs_->Acquire()) }
  );
}
...
void PqxxSessionRepository::DeleteByToken(const std::string& token) {
  ...
  tx.Execute("DELETE FROM sessions WHERE token = $1", QueryParams{}.BindParam(token));
  tx.Commit();
}

void PqxxSessionRepository::DeleteAllForUser(const UserId& userId) {
  auto tx = Transaction{ std::move(connection_pool_obs_->Acquire()) };

  tx.Execute("DELETE FROM sessions WHERE user_id = $1", QueryParams{}.BindParam(userId.ToString()));
  tx.Commit();
}

void PqxxSessionRepository::DeleteExpired() {
  auto tx = Transaction{ std::move(connection_pool_obs_->Acquire()) };

  const std::string sql = "DELETE FROM sessions WHERE expires_at <= $1";

  const auto now = std::chrono::system_clock::now();

  tx.Execute(sql, QueryParams{}.BindParam(now));
  tx.Commit();
}