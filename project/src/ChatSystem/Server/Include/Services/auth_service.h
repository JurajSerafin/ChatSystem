...
private:
  ...
  void HandlePasswordRehash(User& user, const std::string& password) const;
...
template <UserValidatorFor<UserParams> TUserValidator, SessionValidatorFor<SessionParams> TSessionValidator>
void AuthService<TUserValidator, TSessionValidator>::HandlePasswordRehash(User& user, const std::string& password) const {
  if (encryption_service_.NeedsRehash(user.GetPasswordHash())) {
    user.SetPasswordHash(encryption_service_.HashPassword(password));
...
Session AuthService<TUserValidator, TSessionValidator>::Login(const std::string& login, const std::string& password) {
  auto user = user_repo_.FindByLogin(login);
